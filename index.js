...
btns.push(
  ...
  },
  {
    btn: merge_sort_btn,
    sort: () =>
      mergeSort(arr, 0, arr.length - 1).then(() =>
        state.fill(stateColor.sorted.value)
      )
  },
  {
...
btns.forEach(btn => {
  btn.btn.onclick = () => {
    state.fill(
      stateColor.unsorted.value
    ); /* reset the color back to unsorted */
    btn.sort().then(() => enableInputs());
    disableInputs();
  };
});

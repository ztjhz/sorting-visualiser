...
async function merge(arr, start, middle, end) { /* end inclusive */
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = i; j < arr.length; j++) {
            ...
            if (arr[j] < arr[smallIndex]) {
                state[smallIndex] = stateColor.unsorted.value; /* change back to unsorted before swapping */
                smallIndex = j;
                state[smallIndex] = stateColor.pivot.value; /* color the smallest element */
            } else {
                state[j] = stateColor.unsorted.value; /* if not the smallest change back to unsorted color */
            }
        }
